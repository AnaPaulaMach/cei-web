  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-40 border-b-2 border-cei-shadow bg-cei-window/95 shadow-pixel backdrop-blur">
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3">
        <Link className="flex items-center gap-3 font-mono font-black text-cei-shadow" href="/">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          {navLinks.map((link) => (
            <Link className="xp-button rounded-sm px-3 py-2 text-sm font-bold text-cei-shadow" href={link.href} key={link.href}>
              {link.label}
            </Link>
          ))}
        </div>
